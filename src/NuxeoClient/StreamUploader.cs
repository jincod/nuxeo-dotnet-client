...
namespace NuxeoClient
{
    ...
    public class StreamUploader
    {
        ...
        public StreamUploader(Client client)
        {
            this.client = client;
        ...
        public StreamUploader SetChunked(bool isChunkedUpload)
        {
        ...
        /// <returns>The current <see cref="Uploader"/> instance.</returns>
        public StreamUploader SetChunkSize(int size)
        {
            ChunkSize = size;
            return this;
        }
        ...
        /// <returns>The current <see cref="Uploader"/> instance.</returns>
        public StreamUploader SetNumConcurrentUploads(int num)
        {
        ...
        /// <returns>The current <see cref="Uploader"/> instance.</returns>
        public StreamUploader AddFile(string file)
        {
        ...
        /// <returns>The current <see cref="Uploader"/> instance.</returns>
        public StreamUploader AddFiles(string[] files)
        {
        ...
        /// <summary>
        /// Uploads ф file in the upload queue.
        /// </summary>
        /// <returns>A <see cref="Task"/> that will will return an instance of <see cref="EntityList{T}"/> containing
        /// one instance of <see cref="BatchFile"/> per file uploaded.</returns>
        public async Task<(string batchId, EntityList<Entity> entities)> UploadFile(Stream stream, string filename)
        {
            ...
            var job = new StreamUploadJob(blob);
            job.SetFileId(0);
            job.SetChunked(IsChunkedUpload);
            job.SetChunkSize(ChunkSize);
            try
            {
                await Batch.StreamUpload(job);
            }
            catch (ServerErrorException exception)
            {
